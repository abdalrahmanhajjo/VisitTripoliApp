...
async function ensureIndexes(db) {
  ...
  await Promise.all([
    ...
    db.collection('feed_posts').createIndex({ moderation_status: 1, author_role: 1, created_at: -1 }),
    db.collection('feed_posts').createIndex({ place_id: 1, created_at: -1 }),
    db.collection('feed_posts').createIndex({ author_role: 1, place_id: 1, created_at: -1 }),
    db.collection('feed_likes').createIndex({ post_id: 1, user_id: 1 }, { unique: true }),
    db.collection('feed_likes').createIndex({ user_id: 1, created_at: -1 }),
    db.collection('feed_saves').createIndex({ post_id: 1, user_id: 1 }, { unique: true }),
    db.collection('feed_saves').createIndex({ user_id: 1, created_at: -1 }),
    ...
    db.collection('feed_comment_likes').createIndex({ user_id: 1, created_at: -1 }),
    db.collection('feed_reports').createIndex({ post_id: 1, user_id: 1 }, { unique: true }),
    db.collection('place_translations').createIndex({ place_id: 1, lang: 1 }),
    db.collection('event_translations').createIndex({ event_id: 1, lang: 1 }),
    db.collection('email_verification_tokens').createIndex({ user_id: 1, created_at: -1 }),
    ...
    db.collection('password_reset_tokens').createIndex({ user_id: 1, created_at: -1 }),
