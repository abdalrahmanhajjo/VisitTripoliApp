/**
 * AI routes - Trip planning via free Groq API (direct) or optional n8n webhook.
 * Prefer GROQ_API_KEY in .env for direct, free AI (no n8n). Fall back to N8N_WEBHOOK_URL if set.
 */
...
/**
 * Call Groq API directly. messages = [{ role, content }, ...].
 * Throws on non-OK; error message may include "try again in X.XXs" for rate limits.
 */
...
/**
 * Call Groq with retries on rate limit (429). Waits "try again in X.XXs" from error message.
 */
 ...
 * Body: { prompt?: string, system?: string, user?: string, temperature?, maxTokens? }
 * Uses Groq directly if GROQ_API_KEY is set; otherwise n8n webhook.
 */
router.post('/complete', async (req, res) => {
  if (!isConfigured()) {
    return res.status(503).json({
      error: 'AI not configured',
      detail: 'Set GROQ_API_KEY in backend .env for free AI (get key at console.groq.com), or N8N_WEBHOOK_URL for n8n.',
    });
  }
  ...
  if (useGroqDirect()) {
    ...
    try {
      ...
      if (!text) {
        console.warn('[AI] Groq returned empty text');
      } else {
        console.log('[AI] Groq 200 OK, response length:', text.length);
      }
  ...
  try {
    ...
    if (!n8nResponse.ok) {
      ...
      if (n8nResponse.status === 404) {
        if (/GET request|not registered for GET/i.test(msgStr)) {
          ...
          userError = 'Webhook not registered. Turn the workflow Active in n8n, or set GROQ_API_KEY in .env to use free AI without n8n.';
        }
    ...
    const errorDetail = !trimmed && json?.error?.message ? String(json.error.message).slice(0, 200) : undefined;
    if (!trimmed) {
      console.warn('[AI] n8n 200 but empty text. keys:', json ? Object.keys(json) : []);
    } else {
      console.log('[AI] n8n 200 OK, length:', trimmed.length);
    }
    ...
    return res.status(500).json({
      error: isNetwork ? 'Could not reach n8n. Set GROQ_API_KEY in .env to use free AI without n8n.' : 'AI request failed',
      detail: process.env.NODE_ENV !== 'production' ? err.message : undefined,
    });
  }
});
...
router.get('/test', async (req, res) => {
  if (!isConfigured()) {
    return res.status(503).json({
      ok: false,
      error: 'AI not configured',
      hint: 'Add GROQ_API_KEY to backend/.env (free at console.groq.com) or N8N_WEBHOOK_URL.',
    });
  }
  if (useGroqDirect()) {
    try {
  ...
  try {
    ...
    const text = json?.text ?? (json?.choices?.[0]?.message?.content != null ? String(json.choices[0].message.content) : '');
    ...
    return res.status(500).json({
      ...
      provider: 'n8n',
      error: err.message,
      hint: 'Backend cannot reach n8n. Or set GROQ_API_KEY to use free AI without n8n.',
    });
  }
});
